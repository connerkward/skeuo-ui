...
export async function cutoutPaintToFrame(paintUrl: string): Promise<Blob> {
  const res = await fetch(paintUrl);
...
export async function uploadFrame(id: string, frame: Blob): Promise<string> {
  const r = await fetch(`/api/finalize/${encodeURIComponent(id)}`, {
...
// frameUrl to use: the durable R2 URL after upload, or — in the demo path (paintUrl
// is a data: URL, no R2) — an in-memory object URL of the cut frame.
