#!/usr/bin/env python3
"""genskin_ab — A/B copy of gen12/genskin.py for the abshape experiment (2026-07-09).

Question: does the paint model comply better with OUTLINE guides (A, current) or SOLID
FILLED guide shapes (B) in the templated blueprint?

Changes vs genskin.py (everything else identical):
  * edit() goes through Vertex AI (gemini-3-pro-image-preview, project muser-2605300220,
    global) instead of fal — fal balance is dead. Same underlying model.
  * CLI: genskin_ab.py <spec.json> --cond A|B --seed N  → writes abshape/assets-abshape-<cond>-<seed>/
  * cond B draws every guide shape FILLED solid with its guide colour (same shapes,
    positions, sizes) and applies a minimal 4-substitution prompt diff (outline→filled
    patch wording); the exact diff is recorded in results.json["prompt_diff"].

Original genskin docstring follows:
...
def edit_vertex(bp_path, prompt, seed):
    """Image-edit via Vertex generateContent (same model fal proxied). Returns PNG bytes."""
    ...
    body = {
        ...
        "generationConfig": {
            ...
            "seed": seed,
            "candidateCount": 1,
            "imageConfig": {"aspectRatio": "5:4", "imageSize": "4K"},
        },
    }
...
# cond B prompt diff — the ONLY wording changes vs cond A, each must hit exactly once
PROMPT_DIFF_B = [
    ("COLOURED OUTLINE guides marking",
     "COLOURED SOLID FILLED patches marking"),
    ("The coloured outlines are ALIGNMENT MARKINGS (like masking tape)",
     "The coloured filled patches are ALIGNMENT MARKINGS (like masking tape)"),
    ("the coloured guide outline around EVERY control is",
     "the coloured guide patch covering EVERY control is"),
    ("OVER where each guide outline was",
...
def main():
    ...
    cond = sys.argv[sys.argv.index("--cond") + 1].upper()   # A=outline, B=solid
    seed = int(sys.argv[sys.argv.index("--seed") + 1])
    solid = cond == "B"
    sid = spec["id"]; mode = spec["mode"]
    assert mode == "templated", "abshape A/B only makes sense in templated mode"
    ...
    if mode == "templated":
        ...
        for name, spec_l in layout.items():
            ...
            if kind == "btn":
                ...
                if solid: d.ellipse([x - KNOB_R, y - KNOB_R, x + KNOB_R, y + KNOB_R], fill=col)
        ...
        for i, (k, shp) in enumerate(cells):
            ...
            if shp == "circle":
                box = [cx - KNOB_R, sy - KNOB_R, cx + KNOB_R, sy + KNOB_R]
                d.ellipse(box, fill=col) if solid else d.ellipse(box, outline=col, width=12)
            elif shp == "thumb":
                box = [cx - THUMB_W / 2, sy - THUMB_H / 2, cx + THUMB_W / 2, sy + THUMB_H / 2]
                d.rounded_rectangle(box, radius=THUMB_R, fill=col) if solid else d.rounded_rectangle(box, radius=THUMB_R, outline=col, width=12)
            else:
                box = [cx - TOG_W / 2, sy - TOG_H / 2, cx + TOG_W / 2, sy + TOG_H / 2]
                d.rounded_rectangle(box, radius=TOG_R, fill=col) if solid else d.rounded_rectangle(box, radius=TOG_R, outline=col, width=12)
    else:
    ...
    if solid:
        for old, new in PROMPT_DIFF_B:
            assert prompt.count(old) == 1, f"prompt diff anchor not unique: {old!r}"
            prompt = prompt.replace(old, new)
    res["cond"] = cond
    res["prompt_diff"] = PROMPT_DIFF_B if solid else []
    res["prompt"] = prompt
    json.dump(res, open(os.path.join(OUT, "results.json"), "w"), indent=1)

    if "--blueprint-only" in sys.argv:
        json.dump({**res, "prompt_len": len(prompt)}, open(os.path.join(OUT, "results.json"), "w"), indent=1)
        print(f"[blueprint-only] {sid} cond={cond} seed={seed} prompt {len(prompt)} chars → {bp}")
        return
