#!/usr/bin/env python3
"""build_composites (arm 3 / editors) — paste each editor-model result CROP back onto its full
source skin, using the exact same feathered-blend compositing math as the parent inpaintbake
arm's build_composites.py (which itself reimplements erase12.py:erase_model()'s blend: full-
strength interior, alpha ramps to 0 over the outer ~12% margin). Read-only against the parent
dir; only writes under editors/.

Emits, per skin x model pair, four web assets under editors/web/composite/:
  <skin>__<model>-fullskin-thumb.jpg   full skin, downscaled, crop_box outlined in red
  <skin>__<model>-fullskin-full.jpg    full skin, native res, crop_box outlined in red
  <skin>__<model>-seam-thumb.jpg       crop_box padded 1.5x, downscaled (the seam close-up)
  <skin>__<model>-seam-full.jpg        crop_box padded 1.5x, native res
Plus a BEFORE variant per skin (baked-defect original) and the VERTEX baseline (reused from
arm 1's results/, not touched, not re-spent).
"""
import json, os
...
PARENT = os.path.dirname(HERE)  # inpaintbake/ (read-only)
OUT_DIR = os.path.join(HERE, "web", "composite")
RESULTS_DIR = os.path.join(HERE, "results")          # this arm's own results (editors/results/)
PARENT_RESULTS_DIR = os.path.join(PARENT, "results")  # read-only: vertex baseline lives here

CROPS_META = json.load(open(os.path.join(PARENT, "crops_meta.json")))
...
MODEL_RESULTS_DIR = {
    ...
    "gemini25-flash": RESULTS_DIR,
    "gemini31-flash": RESULTS_DIR,
    "finegrain": RESULTS_DIR,
    "gpt-image-2": RESULTS_DIR,
}
MODEL_EXT = {
    "vertex": "png", "gemini25-flash": "png", "gemini31-flash": "png",
    "finegrain": "jpg", "gpt-image-2": "png",
}
...
def feathered_paste(base_rgb_arr, result_crop_img, crop_box):
    """Same blend as production erase12.py:erase_model() / parent arm's build_composites.py."""
