...
def main():
    ...
    prompt = (
        ...
        "  • SHUFFLE STATES — the OFF and ON switch parts have the SAME OUTER HOUSING SILHOUETTE at the same size, "
        "and MUST BE MIRROR OPPOSITES: in the OFF part the sliding lever/puck sits at ONE end of the housing, and in "
        "the ON part it sits at the OPPOSITE end (if OFF is lever-LEFT then ON is lever-RIGHT). NEVER draw the lever "
        "on the same side in both states — they must be clearly reversed. Put ABSOLUTELY NO 'ON', 'OFF', 'I', 'O' or "
        "any text/letters/words/labels on either switch part or on ANY strip part — no engraved or printed words "
        "anywhere on the loose parts; the state reads from lever position alone.\n"
