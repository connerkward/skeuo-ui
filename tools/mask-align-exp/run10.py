#!/usr/bin/env python3
"""run10 — WILD Y2K design stress test: "MAGMA CORE" alien game handheld. Organic asymmetric
molten-lava blob body (horn nubs, side pod — drawn as a PIL polygon, NOT a rounded rect),
cooling black volcanic-rubber crust with glowing lava-orange veins + gold-chrome piercings.
...
OUT = os.path.join(os.path.dirname(__file__), "assets10"); os.makedirs(OUT, exist_ok=True)

# ---------------------------------------------------------------- design palette (DATA, first)
MATERIAL_IS_DARK = True          # black volcanic crust dominates → pale backdrop
BG = (235,235,238) if MATERIAL_IS_DARK else (22,22,26)
PALETTE = {                      # the design's major colours — keys must stay >=120 from ALL
    "magma_orange": (255,110,20),
    "lava_red":     (230,40,25),
    "ember_yellow": (255,210,60),
    "gold_chrome":  (212,170,40),
    "crust_black":  (25,22,20),
    "rubber_dark":  (45,40,38),
    "backdrop":     BG,
}
...
PROMPT=("Two side-by-side columns of identical size. LEFT column: a BLUEPRINT of an early-2000s Y2K ALIEN "
  "GAME HANDHELD console (there is NO text anywhere — the coloured outlines ARE the guides). The housing is "
  "an ORGANIC ASYMMETRIC BLOB — a molten magma droplet with two small horn nubs on top, a bulging pod on its "
  "right flank and an uneven silhouette all around — deliberately NOT a rectangle, NOT a rounded rectangle, "
  "NOT symmetric. Every control is identified by BOTH its guide colour AND its shape+position: "
  ...
  "rounded rectangle near the top=the SCREEN (blank LCD game display window); the small dots on the right "
  "pod=vent pores. "
  ...
  "LEFT column — paint the FINISHED game handheld and parts as MOLTEN MAGMA COOLING INTO BLACK VOLCANIC "
  "ROCK: the blob body is a matte BLACK VOLCANIC-RUBBER crust, cracked like cooling lava, with GLOWING "
  "MOLTEN-LAVA ORANGE veins and seams bleeding through the crack lines (deep ember orange-red glow ONLY — "
  "never white-hot, never pale yellow), pierced by polished GOLD-CHROME metal: gold bezels, gold socket rims, "
  "and the four raised GOLD-CHROME button facets. The two horn nubs are gold-capped. Every surface of the "
  ...
  "  • The device and every control/part must contain NONE of the guide colours — the finished palette is "
  "ONLY black volcanic rubber, glowing lava orange-red, and gold chrome. ABSOLUTELY "
  f"{NO_LIST} paint ANYWHERE in the left column — the painted transport buttons are BARE GOLD-CHROME facets; "
  ...
  "  • The 4 transport buttons are MOLDED into the body as raised gold-chrome facets that KEEP their EXACT "
  ...
  "switch OFF, toggle switch ON — finished black-rubber-and-gold-chrome parts on the flat "
  ...
  "cap is a FLAT CIRCLE seen from directly above — show ONLY its round top face with a knurled gold rim and a "
  ...
  f"  • The screen is blank dark glass — remove its {n('screen').lower()} outline, keep it UNCOLOURED (no "
  "tint, no content, no icons, no game graphics) with a slim gold bezel; the vent pores are tiny drilled "
  "holes.\n"
