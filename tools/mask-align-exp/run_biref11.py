#!/usr/bin/env python3
"""ONE global BiRefNet pass over the run10 (magma game handheld) paint, then SPLIT the matte
into connected-component islands: device body + each sprite part, correlated to identity via
the regions.json strip cells (centroid match, not left-to-right order). Saves every island."""
...
def fal_key():
    ...
    sys.exit("no FAL_KEY")
FAL=fal_key()
SRC=os.path.join(os.path.dirname(__file__),"assets11")
OUT=os.path.join(os.path.dirname(__file__),"assets11_biref"); os.makedirs(OUT,exist_ok=True)
